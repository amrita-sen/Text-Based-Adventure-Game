...
/**
 * The TreasureChest class represents a Treasure Chest object that could be available in the room for the player to unlock.
...
{
	public TreasureChest(String description, Pickup content) // Super constructor that takes in a description and a Pickup object as parameters.
	{
		super(description, content);
		super.setIsLocked(getIsLocked());
	}

	public TreasureChest(Pickup content)             // Super constructor that takes in only a Pickup object as a parameter.
	{
		super(content);
		super.setIsLocked(getIsLocked());
	}
